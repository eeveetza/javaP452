...
% Revision History:
% Date            Revision
% 13JUL23         Updated to align with ITU-R P.452-18
...
for iname = 1 : length(filenames)
    ...
    %% Write in SEAMCAT
    nmax = 500;
if (length(p452.path.d)>nmax)
    disp(['Decimating path for ' filename1(1:end-4)])
    nn = length(p452.path.d);
    ...
    p452.path.d = p452.path.d(index);
    p452.path.h = p452.path.h(index);
    p452.path.g = p452.path.g(index);
    p452.path.zone = p452.path.zone(index);
...
fprintf(fid, 'double press = %f;\n', p452.press);
fprintf(fid, 'double temp = %f;\n', p452.temp);
fprintf(fid, 'double phit_e = %f;\n', p452.phit_e);
fprintf(fid, 'double phit_n = %f;\n', p452.phit_n);
fprintf(fid, 'double phir_e = %f;\n', p452.phir_e);
fprintf(fid, 'double phir_n = %f;\n', p452.phir_n);
fprintf(fid, 'double Gt = %f;\n', p452.Gt);
fprintf(fid, 'double Gr = %f;\n', p452.Gr);
...
fprintf(fid, 'double htg = %f;\n', p452.htg);
fprintf(fid, 'double hrg = %f;\n', p452.hrg);
 
fprintf(fid, 'double[] rDist = new double[%d];\n', length(p452.path.d));
fprintf(fid, 'double[] rHeight = new double[%d];\n', length(p452.path.d));
fprintf(fid, 'double[] g = new double[%d];\n', length(p452.path.d));
fprintf(fid, 'int[] iZone = new int[%d];\n', length(p452.path.d));


for ii = 1 : length(p452.path.d)
    fprintf(fid, 'rDist[%d] = %f;\n', ii-1, p452.path.d(ii));
end
for ii = 1 : length(p452.path.d)
    fprintf(fid, 'rHeight[%d] = %f;\n', ii-1, p452.path.h(ii));
end
for ii = 1 : length(p452.path.d)
    fprintf(fid, 'g[%d] = %f;\n', ii-1, p452.path.g(ii));
end
...
%         Lbs(offset + i) = max(Lbs(offset + i), Lbfsg(offset + i));
        ...
        if(flag_createlog==1)
                ...
                if (pathtype == 1)
                    outstr = 'Line of Sight';
                else
                    outstr = 'Trans-Horizon';
                end
           
         fprintf(fidlog, '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%s,%f,%f,%s,%f,%f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f,%.8f \n', ...
...
fprintf(fid, '// Results obtained from the MATLAB implementation of ITU-R P.452-18\n');
fprintf(fid, '// available on the ITU-R SG3 sharefolder\n');
 
 
fprintf(fid, 'double[] expectedResult;\n');
fprintf(fid, 'expectedResult = new double[%d];\n',nrows);
 
for ii = 1:nrows
    fprintf(fid, 'expectedResult[%d] =  %f;\n', ii-1, Lb(ii));
end
